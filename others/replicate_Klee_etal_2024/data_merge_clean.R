...
# weights
svyweights = read_spss("../data/sharew9_rel9-0-0_ALL_datasets_spss/sharew9_rel9-0-0_gv_weights.sav")

## immediate and delayed recall (+ further variables for later validation)
recall7 = read_spss("../data/sharew9_rel9-0-0_ALL_datasets_spss/sharew9_rel9-0-0_gv_health.sav") %>% 
...
cf7 = read_spss("../data/sharew9_rel9-0-0_ALL_datasets_spss/sharew9_rel9-0-0_cf.sav") %>% 
  mutate(vfluency = ifelse(cf010_ >= 0, cf010_, NA)) %>% 
  select(mergeid, vfluency) 

## education
isced7 = read_spss("../data/sharew9_rel9-0-0_ALL_datasets_spss/sharew9_rel9-0-0_gv_isced.sav") %>% 
...
cv7 = read_spss("../data/sharew9_rel9-0-0_ALL_datasets_spss/sharew9_rel9-0-0_cv_r.sav") %>% 
  select(mergeid, age2022, gender, int_year, int_month, deceased, cvresp) %>% 
  mutate(age = ifelse(is.na(age2022), NA, age2022),
...
iadl7 = read_spss("../data/sharew9_rel9-0-0_ALL_datasets_spss/sharew9_rel9-0-0_ph.sav") %>% 
...
iv7 = read_spss("../data/sharew9_rel9-0-0_ALL_datasets_spss/sharew9_rel9-0-0_iv.sav") %>% 
...
expected_prevalence_OECD = read.csv("./20240124_health_at_a_glance.csv") %>% 
...
dat = 
  ...
  left_join(svyweights %>% select(mergeid, cciw_w9))
