...
weighted_class_prev_per_country = function(dat_with_probdem, dat_with_country){
  ### derives population-level prevalence per country weighted by cciw_w9 for Self-reports, LW (Recall & IADL)*, GLM (weighted) IV, Random Forest SMOTE IV and XGBoost SMOTE IV
  ### requires:
  ...
  # set survey design
  dsgn = svydesign(ids = ~1, weights = ~cciw_w9, data = test_dat_baked_temp)
...
sub_figure_6 = function(dem_stat, data, variables){
  ...
  stats_imp = stats_tmp %>% mutate(y.position = rep(c(90, 95, 100, 40, 45, 50, 55, 3.5, 3.8, 4.2, 4.6), length.out = 22))
